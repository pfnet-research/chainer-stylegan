...
class MultiDataset(dataset_mixin.DatasetMixin):
    def __init__(self, datasetConfig, prefix=None, _additional_augmentor_obj=None, return_meta=False, **kwargs):
        if not isinstance(datasetConfig, list):
            self.dlen = 1
            self.datasets = [BaseDataset(datasetConfig, prefix, _additional_augmentor_obj, return_meta, **kwargs)]
            self._len = self.datasets[0]._len 
            self.len_arr = [self._len]
            self.weight = np.asarray([1.0]).astype('f')
        else:
            self.datasets = []
            self._len = 0
            self.dlen = len(datasetConfig)
            self.len_arr = []
            self.weight = []
            for i in range(self.dlen):
                self.datasets.append(BaseDataset(datasetConfig[i], prefix, _additional_augmentor_obj, return_meta, **kwargs))
                self._len += self.datasets[i]._len 
                self.len_arr.append(self.datasets[i]._len)
                self.weight.append(self.datasets[i].weight)

            self.weight = np.asarray(self.weight)
            self.weight /= np.sum(self.weight)
            print("Dataset Weight:", self.weight)

        super().__init__(**kwargs)

    # The order is not ensureed 
    def get_example(self, i):
        di = np.random.choice(self.dlen, p=self.weight)
        j =  np.random.randint(0, self.len_arr[di])
        return self.datasets[di].get_example(j)

    def get_random_example(self):
        # i = np.random.randint(0, self.__len__())
        return self.get_example(0)

    def get_random_images(self, batch_size):
        results = []
        for j in range(batch_size):
            # i = np.random.randint(0, self.__len__())
            img = self.get_example(0)
            if isinstance(img, tuple):
                img, _ = img
            results.append(img)
